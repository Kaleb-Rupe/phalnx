...
| **C-7** | Phase 5/6 primitives missing REJECT tests | NARROWED (only TA-12 + TA-17, not R-1..R-4 or TA-14) | **PARTIALLY RESOLVED** in `c13c6d9`. TA-17 REJECT test added in `tests/missing-coverage.ts:493-546`. TA-12 stable_balance_floor REJECT requires full validate→DeFi→finalize sandwich — DEFERRED to Phase 6.1 task #55. |
