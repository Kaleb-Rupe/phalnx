...
pub fn verify_jupiter_slippage(ix_data: &[u8], max_slippage_bps: u16) -> Result<()> {
    ...
    require!(
        ix_data.len() >= 24,
        AgentShieldError::InvalidJupiterInstruction
    );
    ...
    let end4 = cursor
        .checked_add(4)
        .ok_or(error!(AgentShieldError::InvalidJupiterInstruction))?;
    require!(
        ix_data.len() >= end4,
        AgentShieldError::InvalidJupiterInstruction
    );
    ...
    for _ in 0..vec_len {
        ...
        if size >= 0 {
            ...
            return Err(error!(AgentShieldError::InvalidJupiterInstruction));
        }
    ...
    require!(
        ...
        AgentShieldError::SlippageTooHigh
    );
...
fn skip_option_remaining_accounts_info(data: &[u8], cursor: &mut usize) -> Result<()> {
    ...
    *cursor = cursor
    ...
    if tag == 1 {
        ...
        require!(tag == 0, AgentShieldError::InvalidJupiterInstruction);
    }
    Ok(())
}
...
mod tests {
    ...
    fn trailing_bytes_3_rejected() {
        ...
        assert_eq!(
            result.unwrap_err(),
            anchor_lang::error!(AgentShieldError::InvalidJupiterInstruction)
        );
    }
    ...
    fn slippage_exceeds_policy() {
        ...
        assert_eq!(
            result.unwrap_err(),
            anchor_lang::error!(AgentShieldError::SlippageTooHigh)
        );
    }
