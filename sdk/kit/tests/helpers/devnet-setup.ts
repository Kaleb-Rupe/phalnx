...
} from "../../src/generated/instructions/initializeVault.js";
import {
  getRegisterAgentInstruction,
} from "../../src/generated/instructions/registerAgent.js";
import {
  getDepositFundsInstructionAsync,
} from "../../src/generated/instructions/depositFunds.js";
import {
  harden,
} from "../../src/harden.js";
import {
...
export async function ensureStablecoinBalance(
  ...
  try {
    ...
    // If mint authority doesn't match, tokens must be obtained via faucet/transfer
    throw new Error(
      `Cannot mint tokens for ${mintAddress}. ` +
      `Owner is not the mint authority. Fund the ATA manually.`,
    );
  }
}
...
export async function sendKitTransaction(
  ...
  // Sign using KeyPairSigner interface
  const signFn = (signer as any).modifyAndSignTransactions ?? (signer as any).signTransactions;
  if (!signFn) {
    throw new Error("Signer does not implement a signing method");
  }
